...
def get_image(i:Image, s:int)->list[int]:
    w,h=i.size;aspect_ratio=h/w;nw=w//s;nh=aspect_ratio*nw*0.55;i=i.resize((nw, round(nh)));return i.getdata()
def image_to_ascii_art(ip,bcrgb,s:int,c:str):
    i=Image.open(ip);grayscale=i.convert("L");rgb=i.convert("RGB");p=get_image(grayscale,s);o=get_image(rgb,s);w=rgb.size[0]//s;chars=['@&%QWNM0gB$#DR8mHXKAUbGOpV4d9h6PkqwSE2]ayjxY5Zoen[ult13If}C{iF|(7J)vTLs?z/*cr!+<>;=^,_:\'        ','@#XOhSje1{J?!^- ','█▓░','█',' '][abs(int(c))][::-1if c[0]=='-'else 1];bg=(f'\x1b[48;2;{';'.join([str(bcrgb[i])for i in range(3)])}m'if isinstance(bcrgb,list)else'');new_pixels=[chars[(int(pixel/(255//len(chars)+1)))]for pixel in p];ascii_image=''
